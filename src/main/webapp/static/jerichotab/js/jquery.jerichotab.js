 ...
 * Copyright 2008, 2009 Jericho [ thisnamemeansnothing[at]gmail.com ] 
	...
	#API:
		...
		#$.fn.jerichoTab.addTab(Function):
				...
				***add by jeffen@pactera 2.16/1/18
				*tabFirerRef(String): the domId or classId that makes tab shown in a cookie way
...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        var opts = $.fn.extend({
            ...
            //add by jeffen@pactera 2106/1/17
            //keep tabsetting by cookie
            isCookieTabsetting: false,
            //auto load data by cookie tabs
            isCookieLoadData: false,
            //set Tabsetting to cookie name
            cookieTabsetting: 'jtabsetting',
            cookieTabLast: 'jtablast',
            //set beforeAddTab function
        	fn_beforeAddTab: null,
        ...
        function createJerichoTab() {
            ...
            */
            ...
            $.fn.jerichoTab = {
                ...
                // add option by jeffen@pactera 2016/1/17
                uniqueId: opts.uniqueId,
                isCookieTabsetting: opts.isCookieTabsetting,
                isCookieLoadData: opts.isCookieLoadData,
                cookieTabsetting: opts.cookieTabsetting,
                cookieTabLast: opts.cookieTabLast,
                ...
                addTab: function(tabsetting) {
                	// add by jeffen@pactera 2016/1/17
                	if($.fn.jerichoTab.fn_beforeAddTab){
                		$.fn.jerichoTab.fn_beforeAddTab.call();
                	}
                    ...
                    var ps = $.fn.extend({
                        ...
                        // add tabFirerRef for cookie refirer by jeffen@pactera 2016/1/18
                        tabFirerRef: null,
                    ...
                    //add undefined check by jeffen@pactera 2106/1/17
                    var jerichotabindex;
                    if (ps.tabFirer != null && ps.tabFirer.attr!=undefined) {
                        jerichotabindex = ps.tabFirer.attr('jerichotabindex');
                    ...
                    // add jerichotabindex case for cookie by jeffen@pactera 2016/1/18
                    else {
                    	if($.fn.jerichoTab.isCookieTabsetting && ps.tabFirerRef){
                    		$(ps.tabFirerRef).attr('jerichotabindex', curIndex);
                    	}
                    }
                    if($.fn.jerichoTab.isCookieTabsetting){
	                    if (ps.tabFirerRef != null) {
	                    	jerichotabindex = null;
							$.fn.jerichoTab.curDatalink = ps.data.dataLink;
	                    	$('#'+$.fn.jerichoTab.uniqueId).find('li[tabfirerref="' + ps.tabFirerRef + '"]').each(function(i, n){
							    if($.fn.jerichoTab.curDatalink == $(n).attr("datalink")) jerichotabindex = $(n).attr("jerichotabindex");
							});
							if(jerichotabindex)
								return $.fn.setTabActive(jerichotabindex).adaptSlider().loadData();
	                    }
                    ...
                    */
                    /*var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
	                                    ...
	                                    '<div class="tab_right">&nbsp;</div>' +
	                                '</li>')*/
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:0px"'
                    		
                    		// add by jeffen@pactera 2016/1/18
                    		+ '  tabfirerref="' + ps.tabFirerRef + '"'
                    		+ '  jerichotabindex="' + curIndex + '"'
                    		
                    		+ '  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                            '<div class="tab_left"  style="width:' + (opts.tabWidth - 5) + 'px"  >' +
                                (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                                '<div class="tab_text" title="' + ps.title + '"  style="width:' + (opts.tabWidth - 45 + (ps.iconImg == '' ? 25 : 0)) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                                '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="关闭">&nbsp;</a>' : '') + '</div>' +
                    ...
                    //use an Array named "Tabsetting" to restore the tab information
                    ...
                    tabHash.push({
                        ...
                        // add tab option by jeffen@pactera 2016/1/17
                        tab: ps
                    });
                    	
                    // add cookieTabsetting by jeffen@pactera 2016/1/17
        			if($.fn.jerichoTab.isCookieTabsetting){
        				$.fn.applyCookieTabsetting(tabHash);
        			}
        			if($.fn.jerichoTab.isCookieLoadData){
	        			return newTab.applySlider().setTabActive(curIndex).loadData();
	        		}
            ...
            // add cookieTabsetting by jeffen@pactera 2016/1/17
            if(opts.isCookieTabsetting 
            		&& cookie(opts.cookieTabsetting)!=null 
            		&& cookie(opts.cookieTabsetting)!='null'){
            	opts.tabs = JSON.parse(cookie(opts.cookieTabsetting));
            }
        ...
        // modify activeTabIndex by jeffen@pactera 2106/1/19
        if(opts.isCookieTabsetting)
        	opts.activeTabIndex = (typeof tagGuid == 'undefined' ? 0 : tagGuid);
    ...
    buildIFrame: function(src) {
        return this.each(function() {
            ...
            // add tabFirerRef by jeffen@pactera 2016/1/20
            var tabFirerRef;
            ...
            for (var tab in tabHash) {
                if (tabHash[tab].holderId == $(this).attr('id')) {
                    ...
                    // add tabFirerRef by jeffen@pactera 2016/1/20
                    tabFirerRef = tabHash[tab].tab.tabFirerRef;
                ...
                $(me).animate({ 'opacity': '0', width: '0px' }, 100, function() {
                    ...
                    for (var t in tabHash) {
                        if (tabHash[t].tabId == $(me).attr('id')) {
                        	//add undefined check by jeffen@pactera 2016/1/17
                            if (tabHash[t].tabFirer != null 
                            		&& tabHash[t].tabFirer.removeAttr!=undefined)
    ...
    applyHover: function() {
        return this.each(function() {
            $(this).hover(function() {
                ...
                // add by jeffen@pactera 2016/1/19
            	if($.fn.jerichoTab.fn_afterTabMouseup){
            		$.fn.jerichoTab.fn_afterTabMouseup.call(this, $(this).attr("tabfirerref"));
            	}
            	if($.fn.jerichoTab.cookieTabLast){
			    	cookie($.fn.jerichoTab.cookieTabLast, $(this).attr("tabfirerref"));
    ...
    //remove the loader
    removeLoader: function(tabFirerRef) {
        switch ($.fn.jerichoTab.loader) {
            case 'usebg':
        ...
        // add by jeffen@pactera 2016/1/20
        if($.fn.jerichoTab.fn_afterTabLoad && tabFirerRef){
    		$.fn.jerichoTab.fn_afterTabLoad.call(this, tabFirerRef);
    	}
    },
    
    //add by jeffen@pactera 2016/1/17
    //apply the tabsetting to cookie from tabHash
    applyCookieTabsetting: function(tabHash) {
        var tabs = [];
        $.each(tabHash, function(i, n) {
        	delete n.tab.tabFirer;
            tabs.push(n.tab);
        });
        var JSON_cookieValue = JSON.stringify(tabs);
    	cookie($.fn.jerichoTab.cookieTabsetting, JSON_cookieValue);
    	window.console && window.console.debug('[jqeuery.jerichotab.js]applyCookieTabsetting='
