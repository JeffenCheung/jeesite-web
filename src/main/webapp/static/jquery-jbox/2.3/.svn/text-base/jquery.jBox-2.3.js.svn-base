...
* v2.3.1 fixed : add condition with a.browser by jeffen@pactera 2015/12/18
*/
(function (a) {
	a.jBox = function (b, c) {
		...
		var d = (a.browser && a.browser.msie && parseInt(a.browser.version) < 0x7);
		var e = a('#' + c.id);
		if (e.length > 0x0) {
		...
		if (!c.isTip && !c.isMessager && !c.showScrolling) {
			a(a.browser && a.browser.msie ? 'body' : 'body').attr('style', 'overflow:hidden;padding-right:17px;')
		};
		...
		if (g) {
			...
			j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon' : (c.showIcon == false ? '' : ' ' + c.showIcon)) + '" style="float:left; width:' + i + '; line-height:' + (a.browser && a.browser.msie ? 0x19 : 0x18) + 'px; padding-left:' + (c.showIcon ? 0x12 : 0x5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (c.title == '' ? '&nbsp;' : c.title) + '</div>');
			j.push('</div>')
		};
		...
		var u = a.browser && a.browser.msie ? 'line-height:19px;padding:0px 6px 0px 6px;' : 'padding:0px 10px 0px 10px;';
		...
		var I = function () {
			if (!c.isTip && !c.isMessager) {
				if (a('.jbox-body').length == 0x1) {
					a(a.browser && a.browser.msie ? 'body' : 'body').removeAttr('style')
				};
				J();
				// 解决框架内object和applet被遮住问题
				fade(a('body, iframe'), function(obj, ifr){
	...
	a.jBox.prompt = function (b, c, d, e) {
		...
		var g = e.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + ((a.browser && a.browser.msie && parseInt(a.browser.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var h = [];
		h.push('html:');
	...
	a.jBox.messager = function (b, c, d, e) {
		...
		var h = e.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + ((a.browser && a.browser.msie && parseInt(a.browser.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var i = [];
		i.push('html:');
