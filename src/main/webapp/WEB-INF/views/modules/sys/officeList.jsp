...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// modify office.id to office.parentId by jeffen@pactera 2015/12/28
			var data, rootId;
			if(${fns:getConfigBoolean('widget.ztree.office.async')}){
				data = ${fns:toJson(page.list)}, rootId = "${not empty office.parentId ? office.parentId : '0'}";
			} else {
				data = ${fns:toJson(list)}, rootId = "${not empty office.id ? office.id : '0'}";
			}
			...
			// 双击记录进行编辑
			$("#treeTableList tr").on("dblclick", function() {
			    location.href="${ctx}//sys/office/form?id="+this.id;
			});
			
			// title tipsy
			$("a, table, th, tr, td").tipsy({fade: true, gravity: 'n'});
		});
		function addRow(list, tpl, data, pid, root){
			for (var i=0; i<data.length; i++){
				...
				if ((${fns:jsGetVal('row.parentId')}) == pid){
					...
					if(row.useable=='0'){
						$("#"+row.id).addClass("warning").attr("title", "【警告】本数据不可用，如需取用请编辑为可用状态。");
					}
					if(row.id==userInfo.office.id){
						$("#"+row.id).addClass("info").attr("title", "【提示】本数据为当前用户所在机构部门。");
					}
