	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(${fns:getConfigBoolean('widget.ztree.office.async')}===true){
				cDebug("selectUserToRole.jsp", "ready.setting=" + JSON.stringify(setting));
				refreshOfficeTree("officeTree", setting, true);
			} else {
				officeTree = $.fn.zTree.init($("#officeTree"), setting, officeNodes).expandAll(true);
			}
		...
		function treeOnClick(event, treeId, treeNode, clickFlag){
			cDebug("treeOnClick", "treeId=" + treeId);
			cDebug("treeOnClick", "treeNode.id=" + treeNode.id);
			$.fn.zTree.getZTreeObj(treeId).expandNode(treeNode);
			if("officeTree"==treeId){
				$.get("${ctx}/sys/role/users?officeId=" + treeNode.id, function(userNodes){
					cDebug("treeOnClick", "setting=" + JSON.stringify(setting));
