	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<span class="help-inline">控制器中定义的权限标识(标记单个权限标识，不支持逗号分割)，如：@RequiresPermissions("权限标识")</span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label text-info">附加选项:</label>
			<div class="controls">
				<form:checkbox path="checked" label="生成下级隐藏菜单，标准权限标识组" />
				<span class="help-inline">(查看：view；编辑：edit；IO：io；DBA：dba)</span>
			</div>
		</div>
